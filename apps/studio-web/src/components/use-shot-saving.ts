...
interface UseShotSavingInput {
	onPersonGenerationImported?: (personId: string) => void;
	runs: ScenarioRunRecord[];
	setSnapshot: (updater: (current: AdminSnapshot) => AdminSnapshot) => void;
}

export function useShotSaving({
	onPersonGenerationImported,
	runs,
	setSnapshot,
}: UseShotSavingInput) {
	...
	const saveShot = useCallback(
		async (asset: StudioMediaAsset) => {
			...
			try {
				if (run.inputPersonId) {
					...
					onPersonGenerationImported?.(run.inputPersonId);
		...
		[onPersonGenerationImported, runs, setSnapshot]
	);
