...
export const BASE_MODEL_IDS = BASE_MODELS.map((model) => model.id);
const baseModelById = new Map(BASE_MODELS.map((model) => [model.id, model]));
...
export function getBaseModelLabel(id) {
	const entry = baseModelById.get(id);
	return entry?.label ?? id;
}
export function getBaseModelFamily(id) {
	const entry = baseModelById.get(id);
 ...
 */
export function groupBaseModelsByFamily(ids = BASE_MODEL_IDS) {
...
function normalizeCivitaiValue(value) {
	return value.toLowerCase().replace(civitaiNormalizePattern, " ").trim();
}
const civitaiAliasIndex = BASE_MODELS.flatMap((model) =>
 ...
 */
export function mapCivitaiBaseModel(value) {
	if (!value) {
		return;
	}
