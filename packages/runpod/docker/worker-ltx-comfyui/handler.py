...
def handler(job):
    ...
    try:
        ...
        for node_id, node_output in outputs.items():
            ...
            # VHS_VideoCombine кладёт mp4/webm в key `gifs`, SaveVideo — в
            # `videos`. Базовый upstream-handler смотрит только `images` и
            # теряет видеофайл. Обработка идентична ветке `images`.
                    ...
                    for video_info in node_output[video_key]:
                        ...
                        if os.environ.get("BUCKET_ENDPOINT_URL"):
                            try:
                                ...
                                print(f"worker-comfyui - Uploading {v_filename} to S3...")
                                ...
                                error_msg = f"Error uploading {v_filename} to S3: {e}"
                                print(f"worker-comfyui - {error_msg}")
                                errors.append(error_msg)
                        else:
                            try:
                                ...
                                print(f"worker-comfyui - Encoded {v_filename} as base64")
                            except Exception as e:
                                error_msg = f"Error encoding {v_filename} to base64: {e}"
                                print(f"worker-comfyui - {error_msg}")
                                errors.append(error_msg)
...
# ---------------------------------------------------------------------------
# LTX worker extensions: per-request Civitai LoRA provisioning + diagnostics
# ---------------------------------------------------------------------------
# Network volume — единый источник правды для всех статических моделей (база,
# audio VAE, spatial upscaler, distill LoRA): заливаются один раз через
# `scripts/seed-ltx-aux-models.ts` / `seed-sulphur-volumes.ts`.
#
# Единственное, что не может быть pre-seeded — это ДИНАМИЧЕСКИЕ Civitai-LoRA,
# которые выбираются per-scenario (`loraCivitaiModelId` в параметрах). Граф
# ссылается на них по имени `civitai-{modelId}-{versionId}.safetensors`; если
# файла нет, ComfyUI отвергает граф на валидации `/prompt` и job возвращает
# `success_no_images`. Поэтому докачиваем недостающие LoRA на общий volume
# перед инференсом — первая генерация прогревает LoRA для всех будущих.
...
def _ltx_read_sentinel():
    for candidate in (
        ...
        try:
            ...
            return {"_sentinel_error": str(e), "path": candidate}
    return {
...
def _ltx_collect_lora_names(job):
    names = set()
    try:
        wf = (job or {}).get("input", {}).get("workflow", {})
...
def _ltx_download_civitai(version_id, dest_path):
    ...
    if token:
        url += f"?token={token}"
    req = urllib.request.Request(url, headers={"User-Agent": "ltx-worker/1.0"})
    fd, tmp = tempfile.mkstemp(suffix=".safetensors", dir=os.path.dirname(dest_path))
    os.close(fd)
    try:
        with urllib.request.urlopen(req, timeout=900) as resp:
            ...
            if "text/html" in ctype:
                raise RuntimeError(
                    f"civitai returned HTML (token/auth issue), content-type={ctype}"
                )
            with open(tmp, "wb") as out:
                _ltx_shutil.copyfileobj(resp, out, 1024 * 1024)
        size = os.path.getsize(tmp)
        if size < _LTX_MIN_LORA_BYTES:
            raise RuntimeError(f"downloaded file too small ({size} bytes)")
        os.replace(tmp, dest_path)
        return {"versionId": version_id, "bytes": size, "status": "downloaded"}
    finally:
        try:
...
def _ltx_ensure_loras(job):
    report = []
    names = _ltx_collect_lora_names(job)
    if not names:
        return report
    lora_dir = _ltx_lora_dir()
    for name in sorted(names):
        m = _LTX_CIVITAI_RE.match(name)
        if not m:
            continue
        version = m.group(2)
        dest = os.path.join(lora_dir, name)
        try:
            if os.path.exists(dest) and os.path.getsize(dest) > _LTX_MIN_LORA_BYTES:
                report.append({"name": name, "status": "cached"})
                continue
            info = _ltx_download_civitai(version, dest)
            info["name"] = name
            report.append(info)
        except Exception as e:
...
_ltx_inner_handler = handler


def handler(job):
    """Wrap upstream handler: provision dynamic LoRA + attach diagnostics."""
    lora_report = []
    try:
        lora_report = _ltx_ensure_loras(job)
    except Exception as e:
        lora_report = [{"status": "fatal", "error": str(e)}]
    try:
        result = _ltx_inner_handler(job)
    except Exception as e:
        result = {"error": f"handler raised: {e}"}
    if isinstance(result, dict):
        try:
            result["_bootstrap_sentinel"] = _ltx_read_sentinel()
            result["_ltx_lora_downloads"] = lora_report
        except Exception as e:
            result["_bootstrap_sentinel_error"] = str(e)
    return result
