...
import type { DomainName, RuntimeConfigSnapshot } from "./domains";
...
export interface RuntimeConfigClientOptions {
	/** Base URL of admin-api, e.g. "https://admin-api.gen.balkhaev.com". */
	adminApiUrl: string;
	/** Defaults to 60_000. */
	cacheTtlMs?: number;
	fetchImpl?: (
	...
	/** Bearer token shared with admin-api for the internal endpoint. */
	internalToken: string;
	logger?: Pick<Console, "warn" | "error">;
	/** Redis URL for invalidation pub/sub. Optional but recommended. */
	redisUrl?: string;
	/** Defaults to 3_000. */
...
export interface RuntimeConfigClient {
	close(): Promise<void>;
	get(domain: DomainName): Promise<RuntimeConfigSnapshot>;
	invalidate(domain: DomainName): void;
}
...
export function createRuntimeConfigClient(
	...
	const cache = new Map<DomainName, CacheEntry>();
	const inflight = new Map<DomainName, Promise<RuntimeConfigSnapshot>>();
	...
	async function fetchSnapshot(
		...
		try {
			...
			return (await response.json()) as RuntimeConfigSnapshot;
		} finally {
			clearTimeout(timer);
		}
	}

	function getInternal(domain: DomainName): Promise<RuntimeConfigSnapshot> {
		const now = Date.now();
