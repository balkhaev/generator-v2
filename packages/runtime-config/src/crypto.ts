...
export interface EncryptedValue {
	/** base64(ciphertext || authTag) — auth tag is the last 16 bytes. */
	ciphertext: string;
	/** base64(iv), 12 bytes. */
	iv: string;
}
...
export function encrypt(plaintext: string, masterKey: Buffer): EncryptedValue {
...
export function decrypt(value: EncryptedValue, masterKey: Buffer): string {
	if (masterKey.length !== KEY_BYTES) {
		throw new InvalidMasterKeyError(
			`expected ${KEY_BYTES}-byte key, got ${masterKey.length}`
		);
